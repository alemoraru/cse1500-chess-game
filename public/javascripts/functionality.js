    ...
    for (var i = 1; i <= 8; i++)
        for (var j = 1; j <= 8; j++) {
            ...
            document.getElementById(currCell).
                classList.
                remove("attackedwhite");
            document.getElementById(currCell).
                classList.
                remove("attackedblack");
        }
}
...
function isKingAttacked() {
    ...
    if (document.getElementById(kw).
        classList.
        contains("attackedblack"))
        return ("white");
    if (document.getElementById(kb).
        classList.
        contains("attackedwhite"))
        return ("black");
    return ("none");
}
...
function isAttackedByWhite(toi, toj) {
    if (document.getElementById(toj + toi).
        classList.
        contains("attackedwhite"))
        return true;
    return false;
...
function move(from, to, callback) {
    ...
    if (document.getElementById(to).style.backgroundImage != "none") {
        ...
        $newImage = $('<img>');
        if (t.includes("knb") || t.includes("knw"))
            $newImage.attr('src', document.getElementById(to).
                style.
                backgroundImage.
                substring(5, 22));
        else
...
function pawnAttacking(fromi, fromj, toi, toj, colorF) {
    var startChar = JSON.stringify(fromj).
        charCodeAt(1);
    var stopChar = JSON.stringify(toj).
        charCodeAt(1);
        ...
        if (isOccupied(toi, toj)) {
            if (fromi === toi)
                if (stopChar - startChar > 0)
                    return recTestRoot(fromi, fromj, toi, String.fromCharCode(JSON.stringify(toj).
                        charCodeAt(1) - 1), colorT);
                else
                    return recTestRoot(fromi, fromj, toi, String.fromCharCode(JSON.stringify(toj).
                        charCodeAt(1) + 1), colorT);
            else if (toi - fromi > 0)
    ...
    if (Math.abs(startChar - stopChar) === Math.abs(toi - fromi)) {
        if (isOccupied(toi, toj)) {
            if (parseInt(fromi) < parseInt(toi)) {
                if (stopChar - startChar > 0)
                    return recTestBishop(fromi, fromj, parseInt(toi) - 1, String.fromCharCode(JSON.stringify(toj).
                        charCodeAt(1) - 1), colorT);
                else
                    return recTestBishop(fromi, fromj, parseInt(toi) - 1, String.fromCharCode(JSON.stringify(toj).
                        charCodeAt(1) + 1), colorT);
...
function moveHistory(from, to) {
    ...
    if (historyCountMove % 2 == 0)
        line = "black: ";
    line += JSON.stringify(from).
        substring(1, 3) + "-->" + JSON.stringify(to).
        substring(1, 3);
    x.innerHTML += line;
...
function changePawn(from, to) {
    ...
    if (colorCurrent(from, to) === "white" && JSON.stringify(to).
        charAt(2) === '8' && fromSecond.includes("pw")) {
        // window.alert("promotion white pawn");
        return "whitePromote";
    }
    if (colorCurrent(from, to) === "black" && JSON.stringify(to).
        charAt(2) === '1' && fromSecond.includes("pb")) {
